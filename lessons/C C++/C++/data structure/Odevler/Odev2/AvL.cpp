...
node * AVL::insert(int value,int* dizi,int ArrLen, node*Node){
        ...
        if(!Node){ // node boş ise
            ...
            c +=2;
            d = -1;
            return new node(value,dizi,ArrLen);//new node(value); // verilen değeri yeni bir düğüm olarak ekle
            } 
        else// boş değilse
...
int sonraki = -1; // düğümlerinde dengelendikten sonraki seviye değerlini tutar
